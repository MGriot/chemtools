...
def generate_categorical_plots():
    ...
    for theme in themes:
        ...
        try:
            ...
            filename = f"mosaic_plot_mpl_{theme}.png"
            fig_mpl.savefig(os.path.join(output_dir, filename), bbox_inches='tight')
            plt.close(fig_mpl)
            print(f"  - Saved {filename}")
        except Exception as e:
            print(f"  - Error generating matplotlib mosaic plot for theme {theme}: {e}")

        # Plotly backend (Treemap) - Removed as per instructions to unify on Matplotlib saving/plotting
        # If a plotly specific example is needed, it should be in its own file
        # try:
